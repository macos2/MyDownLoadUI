...
static void my_soup_dl_class_init(MySoupDlClass *klass) {
	...
	g_signal_new("set_name", MY_TYPE_SOUP_DL, G_SIGNAL_RUN_LAST,
			...
			NULL);
	g_signal_new("download_finish",MY_TYPE_SOUP_DL,G_SIGNAL_RUN_LAST,G_STRUCT_OFFSET(MySoupDlClass,download_finish),NULL,NULL,NULL,G_TYPE_NONE,4,G_TYPE_STRING,G_TYPE_STRING,G_TYPE_STRING,G_TYPE_POINTER,NULL);
}
...
void my_soup_dl_watch_moveto_finish_table(MySoupDl *dl, Thread_data *data) {
	...
	gtk_list_store_set(fin_store, &iter, finish_col_uri, w->uri,
			...
			my_download_ui_get_download_state_pixbuf(priv->ui, data->state),
			finish_col_name, filename, finish_col_size, size,
			finish_col_local, w->local, finish_col_thread_data, data,
			finish_col_size_format, t1, finish_col_error, t2,
			finish_col_finish_time, time_format, finish_col_finish_time_unix,
			g_date_time_to_unix(time), -1);
	gtk_tree_row_reference_free(data->row_ref);
	data->row_ref = NULL;
	g_free(t1);
	g_free(t2);
...
gboolean my_soup_dl_watch(MySoupDl *dl) {
	...
	while (l != NULL) {
		...
		case Finish:
			...
			g_signal_emit_by_name(dl,"download_finish",w->uri,w->filename,w->local,&data->user_data,NULL);
		...
		if(data->w!=NULL)g_mutex_unlock(&w->mux);
		l = l->next;
	}
	l = rl;
	while (l != NULL) {
	...
	if (g_async_queue_length(priv->queue) == 0
			&& g_thread_pool_get_num_threads(priv->pool) == 0) {
		priv->source_id = 0;
...
void my_soup_dl_ui_finish_menu_restart(MyDownloadUi *ui,
	...
	gtk_list_store_set(down_store, &iter, down_col_uri, uri, down_col_name,
			filename, down_col_save_local, local, down_col_thread_data,data,-1);
...
void my_soup_dl_ui_down_menu_resume(MyDownloadUi *ui, GtkTreeRowReference *ref,
	...
	if (data->state == Stop) {
		...
		data->state = Retry;
		g_async_queue_push(priv->queue, data);
		gtk_list_store_set(down_store,&iter,down_col_state,Retry,down_col_state_pixbuf,my_download_ui_get_download_state_pixbuf(priv->ui,Retry),-1);
	}
	if (priv->source_id == 0)
		priv->source_id = g_timeout_add(250, my_soup_dl_watch, dl);
}
...
MySoupDl *my_soup_dl_new(SoupSession *session, MyDownloadUi *ui) {
	...
	g_signal_connect(ui, "down-menu-resume", my_soup_dl_ui_down_menu_resume,
			dl);
	g_object_bind_property(ui,"timeout",session,"timeout",G_BINDING_BIDIRECTIONAL);
	return dl;
}
;
